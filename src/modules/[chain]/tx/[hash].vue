...
<template>
    <div>
      <div>
        <overview v-if="tx.tx_response" :chain="props.chain" :tx="tx" :hash="getHash" :class="{ 'hidden':tab !== '1'}" />

      </div>

        <div v-if="tx.tx_response?.code === 0" class="text-[#ffffff] bg-[#131315]/[.8] rounded-[16px] px-4 pt-3 pb-4  mb-4">
            <h2 class="card-title truncate mb-2 font-[OrbitronMedium] font-[400] tracking-[.5px] text-[14px]">
        ...
        <div v-if="tx.tx_response?.code === 0" class="text-[#ffffff] bg-[#131315]/[.8] rounded-[16px] px-4 pt-3 pb-4  mb-4">
            <h2 class="card-title truncate mb-2 text-[14px] font-[400] font-[OrbitronMedium] tracking-[.5px]">JSON</h2>
            <JsonViewer :value="tx" theme="dark"
             style="background: transparent;" copyable boxed sort expand-depth="5"/>
        <!-- {{baseStore.theme}} -->
            </div>
    </div>
</template>
