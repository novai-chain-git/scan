...
<template>
  <div>
    <div
      class="grid gap-5 sm:grid-cols-1 md:grid-cols-3 pb-[20px] text-[#333333]"
    >
      <!--      <div
        class="flex bg-[#ffffff] h-[135px] flex-col items-center justify-center"
      >
        <span>
          <div
            class="relative flex items-center justify-center overflow-hidden rounded w-9 h-9"
          >
            <Icon class="text-success" icon="mdi:trending-up" size="32" />
            <div
              class="absolute top-0 bottom-0 left-0 right-0 opacity-20 bg-success"
            ></div>
          </div>
        </span>
        <span class="mt-2">
          ...
          <div class="text-xs text-center text-[#999999]">{{ $t('staking.inflation') }}</div>
        </span>
      </div>-->
      <div
        class="flex bg-[#ffffff] h-[135px] flex-col items-center justify-center"
      >
        <span>
          <div
            class="relative flex items-center justify-center mr-2 overflow-hidden rounded w-9 h-9"
          >
            <Icon class="text-primary" icon="mdi:lock-open-outline" size="32" />
            <div
              class="absolute top-0 bottom-0 left-0 right-0 opacity-20 bg-primary"
            ></div>
          </div>
        </span>
        <span class="mt-2">
          ...
          <div class="text-xs text-center text-[#999999]">
      ...
      >
        <span>
          ...
          >
            ...
            />
            <div
              class="absolute top-0 bottom-0 left-0 right-0 opacity-20 bg-error"
            ></div>
          </div>
        </span>
        <span class="mt-2">
    ...
    <div class="bg-[#ffffff]">
      ...
      >
        <div class="bg-transparent tabs tabs-boxed text-[#333333]">
          <a
            class="h-10 px-5 text-[16px] leading-10 cursor-pointer uppercase"
            :class="{
              'bg-[#4352D4]  text-[#ffffff] rounded-full': tab === 'featured',
            }"
            @click="tab = 'featured'"
            >{{ $t('staking.popular') }}</a
          >
          <a
            class="h-10 px-5 text-[16px] leading-10 cursor-pointer uppercase"
            :class="{
              'bg-[#4352D4] text-[#ffffff] rounded-full': tab === 'active',
            }"
            @click="tab = 'active'"
            >{{ $t('staking.active') }}</a
          >
          <a
            class="h-10 px-5 text-[16px] leading-10 cursor-pointer uppercase"
            :class="{
              'bg-[#4352D4] text-[#ffffff] rounded-full': tab === 'inactive',
            }"
            @click="tab = 'inactive'"
      ...
      <div class="px-4 pt-3 pb-4">
          ...
          <table class="table w-full staking-table">
            <thead>
              <tr class="border-none text-[16px] text-[#999999]">
                <th
                  scope="col"
                  class="uppercase font-[400] text-[14px]"
            ...
            <tbody>
              ...
              >
                ...
                <td>
                  <div
                    class="relative px-2 py-1 text-[16px] text-[#333333] truncate rounded-full w-fit"
                  >
                    <!-- <span
                      class="absolute inset-x-0 inset-y-0 opacity-10"
                      :class="`bg-${rank}`"
                    ></span> -->
                    {{ i + 1 }}
                  </div>
                ...
                <!-- 👉 24h Changes -->
                <td class="text-[16px] text-right" :class="change24Color(v)">
                  {{ change24Text(v) }}
                </td>
                <!-- 👉 commission -->
                <td class="text-[16px] text-right">
                  {{
                ...
                <td class="text-center">
                  <div
                    v-if="v.jailed"
                    class="gap-2 text-[#fff]/[0.6] h-10 bg-[#F10000] font-medium px-3 new-bg rounded-full text-[#fff]"
                  >
                    {{ $t('staking.jailed') }}
                  </div>
                  <label
                    v-else
                    for="delegate"
                    class="h-10 px-3 capitalize bg-[#4352D4] font-medium rounded-full badge badge-error text-[#fff]"
                    @click="
        ...
        <div class="flex flex-row items-center">
          <div
            class="relative px-4 py-2 mr-2 text-sm text-[#ffffff] truncate w-fit bg-[#f10000]"
          >
            <span
              class="absolute inset-x-0 inset-y-0 opacity-10 bg-error"
            ></span>
            {{ $t('staking.top') }} 33%
          </div>
          <div
            class="relative px-4 py-2 text-sm truncate w-fit text-[#ffffff] bg-[#F1C900]"
          >
            <span
              class="absolute inset-x-0 inset-y-0 opacity-10 bg-warning"
            ></span>
            {{ $t('staking.top') }} 67%
          </div>
