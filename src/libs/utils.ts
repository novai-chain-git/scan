...
export function encryptString(value: string, len = 9) {
  if (value.length <= 18) {
    return value; // 如果字符串长度小于等于18，直接返回原字符串
  }

  // 获取前9位和后9位
  const start = value.slice(0, len);
  const end = value.slice(Math.abs(len) * -1);

  // 计算中间需要替换为 * 的部分长度
  const middleLength = value.length - 18;

  // 构造加密后的字符串
  const encrypted = start + '***' + end;

  return encrypted;
}

export function formatNumberWithCommas(number: any, decimals = 6,symbol = ',') {
  // 确保输入为数字
  ...
  // 添加逗号分隔符
  if(symbol){
    integerPart = integerPart.replace(/\B(?=(\d{3})+(?!\d))/g, symbol);
  }
