...
export class CosmosRestClient extends BaseRestClient<RequestRegistry> {
  ...
  async GetEventsByAddress(query: string, params: any, page?: PageRequest){
    if(!page) page = new PageRequest()    
    return this.request(this.registry.get_events_by_address,params, `${query}&${page.toQueryString()}`);
  }
  ...
  async getContractToken(token: string | number) {
    return this.request(this.registry.api_contract_token, { token });
  }
  async getTokenPrices() {
    return this.request(this.registry.api_token_prices, {});
  }
}
