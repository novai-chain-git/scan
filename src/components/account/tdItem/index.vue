...
import { addresses, getAmount, getToUpperCase, getFormatUnits, GetContract } from "@/libs/web3/index"
  ...
  <div>
        ...
        <template v-if="JudgingData(value.contractData.value.to, addresses.UniswapV2Router01)">
          ...
          <template v-if="value.contractData.value.data.path">
            <Decimals :address="value.contractData.value.data.path[0]"
              :amount="value.contractData.value.data.amount0In || value.contractData.value.data.amount1In">
            </Decimals>
            <br />
            <Decimals :address="value.contractData.value.data.path[1]"
              :amount="value.contractData.value.data.amount0Out || value.contractData.value.data.amount1Out"></Decimals>
          </template>
          <template v-else>{{ value.contractData.value.name }}</template>
        </template>

        <template v-else-if="JudgingData(value.contractData.value.to, addresses.ChainConnect)">
          <Decimals :address="value.contractData.value.data.tokenContractAddress"
            :amount="value.contractData.value.data.amount"></Decimals>
        </template>

        <template v-else-if="value.contractData.value && (value.contractData?.value?.eventName == 'ERC1155')">
          {{ value.contractData?.value?.data.to ==
            '0x0000000000000000000000000000000000000000' ? $t('account.Burn') : $t('account.Transfer')}}
        </template>
        <template
          v-else-if="value.contractData.value && (typeof (value.contractData.value.data.value) && value.contractData.value.data.value >= 0)">
         
          <Decimals :address="value.contractData.value.to" :amount="value.contractData.value.data.value"></Decimals>
        </template>
    ...
    <template v-if="type === 'token'">
      <template v-if="value.contractData.type">
        <template v-if="JudgingData(value.contractData.value.to, addresses.UniswapV2Router01)">
          <template v-if="value.contractData.value.data.path">
            <RouterLink class="textjb-lv"
              v-if="JudgingData(value.contractData.value.data.path[0], addresses.novaichain)"
              :to="`/${chain}/token/${value.contractData.value.data.path[0]}`">
              nUSDT
            </RouterLink>
            <span v-else>NOVAI</span>
            <br />
            <RouterLink v-if="JudgingData(value.contractData.value.data.path[1], addresses.novaichain)"
              class="textjb-lv" :to="`/${chain}/token/${value.contractData.value.data.path[1]}`">
              nUSDT
            </RouterLink>
            <span v-else>NOVAI</span>
          </template>
        ...
        <template v-else-if="JudgingData(value.contractData.value.to, addresses.ChainConnect)">
          <nameItem :address="value.contractData.value.data.tokenContractAddress"></nameItem>
        </template>
