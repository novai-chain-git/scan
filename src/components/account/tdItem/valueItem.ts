...
import { addresses,getAmount, getToUpperCase,
   getFormatUnits,GetContract, addressesName,
   getNormalNumber,
    provider } from "@/libs/web3/index"
//通过合约获取 合约名称, 
async function getName(address: string) {
 ...
 // 获取
 // eth链处理
async function ethName(address: string): Promise<any> {
  try {
    //
...
export const nameItem = defineComponent({
  ...
  setup(props) {
    ...
    provider().getCode(props.address).then((code: any) => {
      console.log(code.length,'code.length',props.address)
