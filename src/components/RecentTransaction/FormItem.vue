...
const toAddress = computed(() => {
  ...
  if(props.tx.showErc20 && (props.tx.showErc20Name == 'transferFrom' || props.tx.showErc20Name == 'transfer')) return props.tx.toAddress;
...
const getAmount = computed(()=> {
   if(props.tx.showSwap) return `${props.tx.numTo} ${props.tx.nameTo}/${props.tx.numForm} ${props.tx.nameForm}`;
  ...
  let messages: any = props.tx.tx.body.messages;
  if (messages[0]?.data.value) return Number(formatUnits(messages[0]?.data.value, 18)).toFixed(6);
...
<template>
  <div class="flex items-center justify-between p-[5px] md:p-[10px] border-b-[1px] border-[#FFFFFF]/[.16]">
    <div>
    ...
    <div class="flex-1 text-left">
      ...
      <div>to
        <RouterLink
          class="textjb-lv"
          v-if="toAddress"
          :to="`/${chain}/account/${toAddress}`"
          >{{ toAddress }}</RouterLink
        ></div>
    </div>
